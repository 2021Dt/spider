...
headers = {
    ...
    'cookie': 'Hm_lvt_434c501fe98c1a8ec74b813751d4e3e3=1720599744,1720603351; Hm_lvt_9bcbda9cbf86757998a2339a0437208e=1720599712,1720603669,1720660542; Hm_lvt_c99546cf032aaa5a679230de9a95c7db=1720602336,1720603643,1720660401,1721029978; qpfccr=true; no-alert3=true; tk=-4696388440249958892; sessionid=455zq076jd1zk5tz7wusynt6n0s42x9f',
    'pragma': 'no-cache',
    'priority': 'u=0, i',
...
patten = re.compile(r'<img src="data:image/png;base64,(.*?)" class="img_number (.*?)" style="left:(.*?)px">', re.S)


def decode(dkey, dvalue):
    b64 = base64.b64encode((dkey + dvalue).encode()).decode().replace('=', '').encode()
    return MD5.new(b64).hexdigest()

ans_data = 0
for resp in range(1, 6):
    url = 'https://match.yuanrenxue.cn/api/match/4?page=' + str(resp)
    response = requests.get(url, headers=headers).json()
    display = decode(response['key'], response['value'])
    for td in re.findall(r'<td>(.*?)</td>', response['info'], re.S):
        flag = 11.5
        number = 0
        ans = {}
        for i in patten.findall(td):
            if display != i[1]:
                ans[float(i[2]) + flag] = code_number[i[0]]
                number += 1
                flag += 11.5
        a = 0.0
        for j in [11.5, 23.0, 34.5, 46.0]:
            number -= 1
            a += math.pow(10, number) * float(ans.get(j, 0))
        ans_data += a
print(ans_data)
